# Wiki index

Challenge: **How might we help a neighborhood stay safe during extreme
heat?** Region chosen: **South Korea**, elderly agricultural workers who
farm alone (see [problem-statement.md](problem-statement.md)). Six sources
ingested so far, all in [`sources/`](../sources/).

## Start here

- [Overview](overview.md) — synthesis of everything below, and why this is
  an SDG 13 project.
- [Problem statement](problem-statement.md) — the region and group we've
  committed to: elderly agricultural workers in South Korea.
- [Concept](concept.md) — our proposed solution: an early-warning and
  check-in service for elderly farmers who work alone.

## Regions (entity pages)

- [Los Angeles](los-angeles.md) — county-wide heat health data, 2005–2024.
  From [la-extreme-heat.md](../sources/la-extreme-heat.md).
- [South Korea](south-korea.md) — national heat deaths since 2011, skewed
  elderly and agricultural. From
  [korea-heat-deaths-elderly.md](../sources/korea-heat-deaths-elderly.md).
- [Los Angeles vs. South Korea](la-vs-south-korea.md) — what's comparable
  between the two regions and what isn't. Read this before picking one.

## Concepts

- [SDG 13 targets](sdg-13-targets.md) — the UN targets this challenge maps
  to. From [un-sdg13-targets.md](../sources/un-sdg13-targets.md).
- [National U.S. heat mortality trends](national-heat-trends.md) — rising
  heat death rates by race/ethnicity, 2000–2023, national context (not
  LA-specific). From
  [cdc-us-heat-mortality-trends.md](../sources/cdc-us-heat-mortality-trends.md).
- [Existing programs](existing-programs.md) — what LA County already runs,
  plus Ahmedabad's Heat Action Plan, the one program here with an
  independent mortality evaluation. From
  [la-extreme-heat.md](../sources/la-extreme-heat.md) and
  [ahmedabad-heat-action-plan.md](../sources/ahmedabad-heat-action-plan.md).

## Synthesis

- [Gaps and open questions](gaps-and-open-questions.md) — what this wiki
  can't yet answer, and what to research next.

## Not yet written

- `pitch.md` — Session 2.

## All sources

| File | Covers |
|---|---|
| [la-extreme-heat.md](../sources/la-extreme-heat.md) | LA County heat health data brief |
| [korea-heat-deaths-elderly.md](../sources/korea-heat-deaths-elderly.md) | KDCA heat death statistics |
| [un-sdg13-targets.md](../sources/un-sdg13-targets.md) | Official SDG 13 targets |
| [cdc-us-heat-mortality-trends.md](../sources/cdc-us-heat-mortality-trends.md) | US national heat mortality, 2000–2023 |
| [ahmedabad-heat-action-plan.md](../sources/ahmedabad-heat-action-plan.md) | Ahmedabad's Heat Action Plan |
| [korea-heat-deaths-2025-season-elderly-farmworkers.md](../sources/korea-heat-deaths-2025-season-elderly-farmworkers.md) | 2025-season elderly + farm-work death overlap |

See [log.md](log.md) for the ingest history.
